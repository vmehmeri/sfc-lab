...
def create_sf_container():
    container = docker_cli.create_container(image='vmehmeri/sf')
    return container
...
def stop_sf_container(container):
    response = docker_cli.stop(container=container.get('Id'))
    return response

def remove_sf_container(container):
    response = docker_cli.remove_container(container=container.get('Id'))
    return response
